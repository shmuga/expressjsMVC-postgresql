...
class ErrorFixer(errorhandler.ErrorHandler):
    ...
    def HandleFile(self, filename, first_token):
        ...
        self._file_is_html = filename.endswith('.html') or filename.endswith(
            '.htm')
        ...
        if code == errors.JSDOC_PREFER_QUESTION_TO_PIPE_NULL:
            ...
            if end_quote:
                single_quote_start = Token(
                    "'", Type.SINGLE_QUOTE_STRING_START, token.line,
                    token.line_number)
                ...
                if last_type.type != Type.DOC_END_BRACE:
                    ...
                    if trailing_space:
                        tokenutil.SplitToken(last_type,
                                             len(
                                                 last_type.string) - trailing_space)
            ...
            if match:
                ...
                match.group('leading_whitespace'),
                match.group('email'),
    ...
    def FinishFile(self):
        ...
        if self._file_fix_count:
            ...
            if not f:
                print 'Fixed %d errors in %s' % (
                self._file_fix_count, self._file_name)
